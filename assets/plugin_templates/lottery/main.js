// StarHope 抽奖 & 点名 (shp.sycamorelost.lottery) v5
// 新增：多档抽奖（奖品分一/二/三等，按档各抽 N 个）。
// 既有：奖品管理(权重/限中/排除/必中) · 多策略 · 批量 · 概率进度条 · 方案 · 导入导出 · 点名。
starhope.title = '抽奖 & 点名';

var draft = { name: '', weight: '1', maxCount: '0', tier: '1' };
var rollDraft = '';
var lastResults = [];
var tierResults = {};   // 多档结果 {tier: [names]}
var rollResults = [];
var msg = '';
var tab = 'draw';
...
function prizesTab(){
  ...
  var list = prizes.length ? prizes.map(function(p){
    ...
    return card(null, [
      {type:'row', children: [ body(p.name, null), {type:'spacer'}, muted('已中 ' + (counts[p.id]||0)), {type:'sizedbox', width:6}, {type:'button', icon:'delete', variant:'outlined', onTap:'remove:' + p.id} ]},
      gap(4),
  ...
  return col([
    card('添加奖品', [
      ...
      gap(6),
      h2('档位'),
      {type:'segmented', value:'' + (draft.tier||'1'), options:[{value:'1', label:'一等'},{value:'2', label:'二等'},{value:'3', label:'三等'}], onChanged:'setTier'},
      gap(4),
      muted('限中次数：0 = 无限，>0 = 抽满即移出奖池；档位用于多档抽奖'),
    ]),
    gap(10),
...
function drawTab(){
  ...
  [1,2,3].forEach(function(t){
    var arr = tierResults[t] || [];
    if (arr.length) tierKids.push({type:'row', children: [badge(tierName(t), 'primary'), {type:'sizedbox', width:8}, body(arr.join('、'), null)]});
    tierKids.push(gap(4));
  });
  var prev = gap(0);
  if (getStrategy() === 'weighted' && pool.length) {
    ...
    prev = card('中签概率（按权重）', pool.map(function(p){
      ...
      return {type:'column', crossAxisAlignment:'stretch', children: [ {type:'row', children:[body(p.name, null), {type:'spacer'}, body(pct + '%', 'primary')]}, gap(3), {type:'progress', value:frac, color:'primary'}, gap(6) ]};
    }));
  }
  return col([
    card('抽奖设置', [
    ...
    gap(10),
    card('多档抽奖（按档位各抽 ' + getBatch() + ' 个）', [
      {type:'button', icon:'star', variant:'tonal', label:'按一/二/三等奖各抽', onTap:'drawTiered', expanded:true},
      gap(8),
      muted('从各档位奖品池中分别抽取（不重复模式下抽中移出）'),
      gap(10),
    ].concat(tierKids)),
    gap(10),
    prev,
...
function rollTab(){
  ...
  return col([
    card('点名设置', [
      ...
      pool.length < names.length ? muted('可点 ' + pool.length + ' / ' + names.length) : gap(0),
      gap(6),
      {type:'button', icon:'refresh', variant:'outlined', label:'重置点名记录', onTap:'resetRollDrawn'},
    ]),
    gap(10),
    card(null, [
...
function rollListTab(){
  ...
  var list = names.length ? names.map(function(n){
    ...
    return card(null, [
      {type:'row', children: [ body(n.name, gone ? 'muted' : null), {type:'spacer'}, n.leave ? badge('请假', 'error') : gap(0), {type:'sizedbox', width:4}, {type:'button', icon: n.leave ? 'add':'remove', variant:'outlined', label: n.leave ? '销假':'请假', onTap:'toggleLeave:' + n.id}, {type:'sizedbox', width:4}, {type:'button', icon:'delete', variant:'outlined', onTap:'removeName:' + n.id} ]},
    ]);
  }) : [muted('名单为空，添加人员')];
  return col([
    card('添加人员', [
      {type:'row', children: [ {type:'expanded', child:{type:'textfield', key:'rname', label:'姓名', value:rollDraft, onChanged:'setRollDraft'}}, {type:'sizedbox', width:6}, {type:'button', icon:'add', label:'加', onTap:'addName'} ]},
      gap(6),
      {type:'row', children: [ {type:'expanded', child:{type:'textfield', key:'rbulk', label:'批量输入（逗号或换行分隔）', onChanged:'setBulk'}}, {type:'sizedbox', width:6}, {type:'button', icon:'list', label:'批量加', onTap:'addBulk'} ]},
    ]),
    gap(10),
    card('名单（' + names.length + '）', [{type:'row', children: [ {type:'button', icon:'delete', variant:'outlined', label:'清空名单', onTap:'clearNames'}, {type:'sizedbox', width:6}, {type:'button', icon:'save', variant:'outlined', label:'导出', onTap:'exportNames'}, {type:'sizedbox', width:6}, {type:'button', icon:'add', variant:'outlined', label:'导入', onTap:'importNames'} ]}, gap(6)].concat(list)),
  ]);
}
function historyTab(){
...
function schemeTab(){
  ...
  var list = schemes.length ? schemes.map(function(s, i){
    return card(null, [ {type:'row', children: [ body(s.name + '（' + s.prizes.length + '个奖品）', null), {type:'spacer'}, {type:'button', icon:'list', variant:'outlined', label:'加载', onTap:'loadScheme:' + i}, {type:'sizedbox', width:4}, {type:'button', icon:'delete', variant:'outlined', onTap:'delScheme:' + i} ]} ]);
  }) : [muted('还没有方案')];
  return col([
    card('保存抽奖方案', [ {type:'row', children: [ {type:'expanded', child:{type:'textfield', key:'sn', label:'方案名', onChanged:'setSchemeName'}}, {type:'sizedbox', width:6}, {type:'button', icon:'save', label:'保存', onTap:'saveScheme'} ]}, gap(4), muted('保存当前奖品与抽奖设置') ]),
    gap(10),
...
function poolInfo(){
  ...
  return getPrizes().filter(function(p){
    if (p.exclude) return false;
    if (p.maxCount > 0 && (counts[p.id]||0) >= p.maxCount) return false;
    if (getUnique() && drawn.indexOf(p.id) >= 0) return false;
    return true;
  });
}
...
function onAction(name, args){
  ...
  switch (name) {
    ...
    case 'exportNames': starhope.storage.set('__clip__', JSON.stringify(getNames().map(function(n){ return n.name; }))); msg = '已导出 ' + getNames().length + ' 个姓名'; break;
...
function addPrize(){
  ...
  var p = getPrizes(); p.push({id: uid(), name: n, weight: parseInt(draft.weight)||1, maxCount: parseInt(draft.maxCount)||0, exclude: false, must: false, tier: parseInt(draft.tier)||1});
  savePrizes(p); draft.name = ''; draft.weight = '1'; draft.maxCount = '0'; draft.tier = '1';
}
...
function saveScheme(){
  ...
  var s = getSchemes(); s.push({name: schemeName, prizes: getPrizes().map(function(p){ return assign(p, {}); }), strategy: getStrategy(), unique: getUnique(), batch: getBatch()});
  starhope.storage.set('schemes', s); schemeName = '';
}
...
function importJson(){
  var raw = starhope.storage.get('__clip__'); if (!raw) { msg = '剪贴板为空'; return; }
  try { var arr = JSON.parse(raw); if (!arr || arr.length === undefined) throw 'x';
    var p = getPrizes(); arr.forEach(function(x){ p.push({id: uid(), name: x.name||'?', weight: parseInt(x.weight)||1, maxCount: parseInt(x.maxCount)||0, exclude: false, must: false, tier: parseInt(x.tier)||1}); });
    savePrizes(p); msg = '已导入 ' + arr.length + ' 个奖品';
  } catch(e) { msg = '导入失败：非有效 JSON 数组'; }
}
function drawN(pool, n, strategy){
  var avail = pool.slice(), winners = [];
  for (var i = 0; i < n && avail.length; i++) {
    var idx;
    if (strategy === 'weighted') {
      var total = avail.reduce(function(a, p){ return a + (parseInt(p.weight)||1); }, 0);
      var r = Math.random() * total, acc = 0; idx = avail.length - 1;
      for (var j = 0; j < avail.length; j++) { acc += (parseInt(avail[j].weight)||1); if (r < acc) { idx = j; break; } }
    } else { idx = Math.floor(Math.random() * avail.length); }
    winners.push(avail[idx]); avail.splice(idx, 1);
  }
  return winners;
}
function recordWinners(winners){
  var h = getHistory(); h.unshift({time: nowStr(), names: winners.map(function(p){ return p.name; })}); if (h.length > 100) h = h.slice(0, 100);
  starhope.storage.set('history', h);
  var counts = getCounts(), drawn = getDrawn();
  winners.forEach(function(p){ counts[p.id] = (counts[p.id]||0) + 1; if (getUnique()) drawn.push(p.id); });
  starhope.storage.set('counts', counts); starhope.storage.set('drawn', drawn);
}
function draw(){
  var pool = poolInfo();
  if (!pool.length) { msg = '奖池为空'; return; }
  var must = pool.filter(function(p){ return p.must; });
  var rest = pool.filter(function(p){ return !p.must; });
  var batch = Math.min(getBatch(), pool.length);
  var winners = must.slice(0, batch);
  var need = batch - winners.length;
  winners = winners.concat(drawN(rest, need, getStrategy()));
  lastResults = winners.map(function(p){ return p.name; });
...
function drawTiered(){
  ...
  var any = false;
  [1,2,3].forEach(function(t){
    var tpool = all.filter(function(p){ return (p.tier||1) === t && !p.exclude && !(p.maxCount > 0 && (counts[p.id]||0) >= p.maxCount) && !(getUnique() && drawn.indexOf(p.id) >= 0); });
    if (tpool.length) {
      var winners = drawN(tpool, Math.min(getBatch(), tpool.length), getStrategy());
      tierResults[t] = winners.map(function(p){ return p.name; });
      recordWinners(winners);
      any = true;
    }
  });
  if (!any) { msg = '无可抽取的档位奖品'; }
}

function addName(){
  var n = (rollDraft || '').trim();
  if (!n) { msg = '姓名不能为空'; return; }
  var names = getNames(); names.push({id: uid(), name: n, leave: false}); saveNames(names); rollDraft = '';
}
function addBulk(){
  var parts = (bulkDraft || '').split(/[，,\n]/).map(function(s){ return s.trim(); }).filter(function(s){ return s; });
  if (!parts.length) { msg = '批量输入为空'; return; }
  var names = getNames(); parts.forEach(function(s){ names.push({id: uid(), name: s, leave: false}); });
  saveNames(names); bulkDraft = ''; msg = '已批量添加 ' + parts.length + ' 人';
}
function importNames(){
  var raw = starhope.storage.get('__clip__'); if (!raw) { msg = '剪贴板为空'; return; }
  try { var arr = JSON.parse(raw); if (!arr) throw 'x';
    var names = getNames();
    (arr.length !== undefined ? arr : [arr]).forEach(function(s){ names.push({id: uid(), name: typeof s === 'string' ? s : (s.name||'?'), leave: false}); });
    saveNames(names); msg = '已导入 ' + (arr.length !== undefined ? arr.length : 1) + ' 人';
  } catch(e) { msg = '导入失败：非有效 JSON（姓名数组）'; }
}
function rollCall(){
  var pool = rollPool();
  if (!pool.length) { msg = '可点名单为空'; rollResults = []; return; }
  var batch = Math.min(getRollBatch(), pool.length);
  var winners = drawN(pool, batch, 'uniform');
  rollResults = winners.map(function(n){ return n.name; });
  var h = getRollHistory(); h.unshift({time: nowStr(), names: rollResults.slice()}); if (h.length > 100) h = h.slice(0, 100);
  starhope.storage.set('rollHistory', h);
  if (getRollUnique()) { var rd = getRollDrawn(); winners.forEach(function(n){ rd.push(n.id); }); starhope.storage.set('rollDrawn', rd); }
}
