...
function rollListTab(){
  ...
  return col([ card('添加人员', [ {type:'row', children:[ {type:'expanded', child:{type:'textfield', key:'rname', label:'姓名', value:rollDraft, onChanged:'setRollDraft'}}, {type:'sizedbox', width:6}, {type:'button', icon:'add', label:'加', onTap:'addName'} ]}, gap(6), {type:'row', children:[ {type:'expanded', child:{type:'textfield', key:'rbulk', label:'批量输入（逗号或换行分隔）', onChanged:'setBulk'}}, {type:'sizedbox', width:6}, {type:'button', icon:'list', label:'批量加', onTap:'addBulk'} ]} ]), gap(12), card('名单（' + names.length + '）', [{type:'row', children:[ {type:'button', icon:'delete', variant:'outlined', label:'清空名单', onTap:'clearNames'}, {type:'sizedbox', width:6}, {type:'button', icon:'save', variant:'outlined', label:'导出', onTap:'exportNames'}, {type:'sizedbox', width:6}, {type:'button', icon:'add', variant:'outlined', label:'导入', onTap:'importNames'} ]}, gap(6), muted('点星标循环切换权重（1→2→3→1）')].concat(list)) ]);
}
function historyTab(){
...
function schemeTab(){
  ...
  return col([ card('保存抽奖方案', [ {type:'row', children:[ {type:'expanded', child:{type:'textfield', key:'sn', label:'方案名', onChanged:'setSchemeName'}}, {type:'sizedbox', width:6}, {type:'button', icon:'save', label:'保存', onTap:'saveScheme'} ]}, gap(4), muted('保存当前奖品与抽奖设置') ]), gap(12), card('数据快照（全量导入导出）', [
      {type:'button', icon:'save', variant:'tonal', label:'导出完整快照', onTap:'exportSnapshot', expanded:true},
      gap(6),
      {type:'button', icon:'add', variant:'outlined', label:'从快照恢复', onTap:'importSnapshot', expanded:true},
      gap(10),
      muted('快照包含奖品+名单+历史+设置，content_type 标记类型'),
      gap(10)
    ]),
    gap(12),
    card('奖品导入/导出', [ {type:'row', children:[ {type:'expanded', child:{type:'button', icon:'save', variant:'outlined', label:'导出 JSON', onTap:'exportJson'}}, {type:'sizedbox', width:6}, {type:'expanded', child:{type:'button', icon:'add', variant:'outlined', label:'导入 JSON', onTap:'importJson'}} ]} ]), gap(12), card('已存方案（' + schemes.length + '）', list) ]);
}
...
function spinDraw(){
  var pool = poolInfo();
  if (!pool.length) { msg = '奖池为空'; starhope.rerender(); return; }
  var spins = 0, maxSpins = 10;
  function step(){ if (spins < maxSpins) { lastResults = [pool[Math.floor(Math.random()*pool.length)].name]; msg = '滚动中...'; starhope.rerender(); spins++; setTimeout(step, 70 + spins * 20); } else { draw(); starhope.rerender(); } }
  step();
}
function spinRoll(){
  var pool = rollPool();
  if (!pool.length) { msg = '可点名单为空'; rollResults = []; starhope.rerender(); return; }
  var spins = 0, maxSpins = 10;
  function step(){ if (spins < maxSpins) { rollResults = [pool[Math.floor(Math.random()*pool.length)].name]; msg = '滚动中...'; starhope.rerender(); spins++; setTimeout(step, 70 + spins * 20); } else { rollCall(); starhope.rerender(); } }
  step();
}
...
function importNames(){ var raw = starhope.storage.get('__clip__'); if (!raw) { msg = '剪贴板为空'; return; } raw = raw.trim(); var names = getNames(); var added = 0; try { var arr = JSON.parse(raw); if (arr) { (arr.length !== undefined ? arr : [arr]).forEach(function(s){ names.push({id: uid(), name: typeof s === 'string' ? s : (s.name||s['姓名']||'?'), leave: false, weight: typeof s === 'object' ? (parseInt(s.weight)||parseInt(s['权重'])||1) : 1}); added++; }); } } catch(e) { var parts = raw.split(/[，,\n\t;；]/).map(function(s){ return s.trim(); }).filter(function(s){ return s; }); parts.forEach(function(s){ names.push({id: uid(), name: s, leave: false, weight: 1}); added++; }); } saveNames(names); msg = '已导入 ' + added + ' 人'; }
function undoDraw(){
...
function exportSnapshot(){
  ...
  msg = '已导出完整快照（奖品 ' + getPrizes().length + ' + 名单 ' + getNames().length + ' + 历史 + 设置）';
}
function importSnapshot(){
  var raw = starhope.storage.get('__clip__'); if (!raw) { msg = '剪贴板为空'; return; }
  try {
    var data = JSON.parse(raw);
    if (data.content_type !== 'lottery_full_snapshot') { msg = '非抽奖快照格式'; return; }
