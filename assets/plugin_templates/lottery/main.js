...
function rollPool(){
  var rollDrawn = getRollDrawn();
  return getNames().filter(function(n){ return !n.leave && !(getRollUnique() && rollDrawn.indexOf(n.id) >= 0); });
}
...
function importJson(){
  ...
  try { var arr = JSON.parse(raw); if (!arr || arr.length === undefined) throw 'x';
    var p = getPrizes(); arr.forEach(function(x){ p.push({id: uid(), name: x.name||'?', weight: parseInt(x.weight)||1, maxCount: parseInt(x.maxCount)||0, exclude: false, must: false}); });
    savePrizes(p); msg = '已导入 ' + arr.length + ' 个奖品';
  } catch(e) { msg = '导入失败：非有效 JSON 数组'; }
}
function draw(){
  var pool = poolInfo();
  if (!pool.length) { msg = '奖池为空'; lastResults = []; return; }
  var must = pool.filter(function(p){ return p.must; });
  var rest = pool.filter(function(p){ return !p.must; });
  var batch = Math.min(getBatch(), pool.length);
  var winners = must.slice(0, batch);
  var need = batch - winners.length, avail = rest.slice();
  for (var i = 0; i < need && avail.length; i++) {
    var idx;
  ...
  var h = getHistory(); h.unshift({time: nowStr(), names: lastResults.slice()}); if (h.length > 100) h = h.slice(0, 100);
