...
> **更新（2026-07-23 · 插件阶段3 + 抽奖插件 v15）**：插件系统**阶段3（JS 引擎）完成**——`flutter_js`(QuickJS) + `PluginRuntime`（eval render/onAction，注入 `starhope.{log,storage,rerender,sha256}`）+ `widget_view` JSON→Flutter 渲染（组件：column/row/wrap/card/text/button/textfield/segmented/checkbox/progress/badge/slider/empty/error + **渲染器注册表 `registerWidget` 扩展点**）+ `plugin_page`（FrostedBackground+返回+KeyboardListener Space 快捷键）。**抽奖&点名插件 v15**（`shp.sycamorelost.lottery`，pls 目录）：滚动动画抽取 / Space 快捷键 / 多档抽奖(一/二/三等) / 多策略(均匀/加权/不重复) / 批量 / 权重 / 限中次数 / 排除 / 必中 / 概率进度条 / 中签统计 / 撤销 / 多格式导入(JSON/CSV/TXT+中文keyMap) / 模块化快照导出(content_type 路由) / 三级空状态 / slider / in-band 错误 / scheme 完整 preset(含 roll 设置) / 点名加权/请假/随机分组 / 历史导出 / 退出按钮。字号四级(18/14/13/11)无突兀。改动仅限插件文件(main.js+widget_view)，StarHope 本身未动。
> **更新（2026-07-23 · 托盘/插件/导航）**：(1) **系统托盘**——关闭窗口可最小化到托盘（`tray_manager`，首次询问并记住），`WindowService.hide/show`（win32），自动锁定开关（`ThemeProvider.lockOnHide`）；(2) **插件系统骨架（阶段 2）**——`.starhope-plugin` 包格式（严格署名：`app=StarHope` + `shp.` 专有名 + SHA-256 防篡改，作者可选担署名），DB v8 `plugins` 表 + `<dataRoot>/plugins/<id>/`，`PluginService/Provider`（安装/卸载/启用/参数/扫描对账/导出），插件工具箱页（3 列毛玻璃卡 / 导入导出 / 批量 / 参数 / 图标点详情 / 下载模板与规则）；**不含 JS 执行**（阶段 3 接 `flutter_js`）；(3) **底部导航重构**——删 NavigationRail，`StarBottomNav` 圆边毛玻璃 5 项（学习工具/插件/摘要[圆形头像]/AI/设置，默认摘要首页），学习工具页 5 玻璃块（题库/练习/考试/错题本/阅读）push 进入，摘要页承担账号资料编辑（迁自设置）。db v7→v8、新增 `tray_manager` 依赖（ATL patch 未受影响）。
> **更新（2026-07-23 · 插件 v16 + 数据模块化 + 全量性能/规范优化）**：
> (1) **抽奖&点名插件 v16**（UI 全面打磨）：复选框标签正常字号(13pt，修「不重复抽取/点名」相对显大)；单段式 Tab；添加奖品/人员控件重排；全局消息条(error/success/info)；**新增宿主 API `starhope.openDataDir()`**（仅打开本插件自身数据目录、无路径参数=无滥用面；manifest 加 `open_dir` 权限；方案 tab「打开存储数据位置」入口）；widget_view 复选框 Flexible+显式字号、补图标(undo/folder/close/open)。运行副本已同步（`<dataRoot>/plugins/shp.sycamorelost.lottery/`，未碰 storage.json）。Node debug 测试全过。
> (2) **数据管理三处模块化**：新增 `lib/services/data_modules.dart` 的 `DataModule` 统一注册表（备份/恢复/删除单一数据源：id/label/tables/payloadKey，图标留视图层）；**备份新增 plugins 模块**（DB 登记行 + plugins/<id>/ 全部文件含 storage.json）；**恢复改选择性**（`restoreBackup(file,{modules})`，仅清空所选模块表+磁盘，修了 partial 备份全选会 clearAll 全表的数据丢失隐患）；**删除复用注册表 + 插件磁盘清理**（`PluginService.deleteAllDirs`）。`_pickBackupModules` 三处复用，恢复时仅列备份实有模块。
> (3) **性能优化（双代理审计后）**：① 头像/附件热路径 `CachedFileCircleAvatar`（stat-once + 缓存 FileImage，聊天卡顿主因）；② OptionTile `FormulaSyntax`/builder 提 static final；③ hoist 内联 FutureBuilder（settings dataRoot initState / practice 按(session,idx)缓存取题 / practice summary 全表按 session 缓存 / PluginProvider 图标字节缓存）；④ wrong_book `_filteredSorted` 按签名 memo + reader 笔记侧栏分组按 notes 身份 memo。
> (4) **规范/分层清理**：删 8 处防 lint 占位死代码(_kApp/_kVer/_kFmtVersion/_ensureCryptoLinked)+随之真正未用的导入（注意：constants.dart 同时含 QuestionType/ShareContentType 枚举，多用途导入需保留）；`registerWidget` 从私有 State 迁至公开 `PluginWidgetRegistry`（消 unused 警告+扩展点可用）；2 处空 catch 补 debugPrint；`formula_render.dart` 从 core git mv 至 views/common（构建 Widget，属视图层）。
> (5) **暂缓项**：上帝类拆分（models 1156 / AppDatabase 951 / 各页 1100+）属纯组织性、UI 状态机拆分回归风险高，未在仅 build+启动验证下保证零影响，留作后续（零回归做法：models 用 barrel re-export、AppDatabase 用 facade 保留所有 instance 方法）；snackBar try/catch helper(11+处) 收益不抵改动风险，留作小项；视图层 18 处直调 AppDatabase.instance 越层，留作后续。
> **验证**：`flutter analyze` **0 issues**（含此前预存 registerWidget 警告已消除）；`flutter build windows` 通过；`timeout 8` exit=124。本会话 8 个细粒度提交（plugin v16 / backup 模块化 / 4 个 perf / 2 个 refactor）。
> **更新（2026-07-23 · 补充轮）**：(1) **抽奖&点名插件 v17**：新增「模板」tab（奖品/名单/完整快照三套严格 JSON 格式 + 字段说明 + 一键复制模板 copyPrizeTemplate/copyNameTemplate/copySnapshotTemplate）；中奖/点名结果改为**独立大号毛玻璃结果框** `resultCard`（页面焦点位置，滚动中翻滚 36pt 大字、结束后单 34pt/多 24pt，替代原 18pt 内联文字）；spinDraw/spinRoll 用 `rolling` 标志驱动结果框翻滚态。运行副本已同步 v17（保留 storage.json）。Node debug 全过。(2) **Bug 修复**：错题本「练这些错题」/ 考试「重练错题」用 `Navigator.push(PracticePage())` 作为独立路由，脱离 home_shell 的 FrostedBackground → 透明 Scaffold 显黑背景；修复：push 时 `FrostedBackground(child: PracticePage())` 包裹（wrong_book_page + exam_page 两处同型）。(3) **免责声明**新增「八、第三方插件」条款（沙箱能力+第三方风险由用户担），后续条款顺延，署名/防伪不变。验证同上 0 issues + build + 124。
> **更新（2026-07-23 · 插件 v18）**：抽奖&点名插件内容**超一屏可垂直滚动**——`widget_view` 新增 `scroll` 节点类型(SingleChildScrollView)，插件 `render()` 根包入 `{type:scroll,child:col(...)}`，各 tab 长列表(奖品/名单/历史/模板)不再溢出截断。纵向根列无 Expanded(16 处全在横向 row)，滚动视图安全。版本 bump 17→18(sha256/size 同步)，运行副本已同步(保留 storage.json)。analyze 0 / build 通过 / 启动 124。
> **更新（2026-07-23 · 插件 v19）**：抽奖&点名插件**导入导出真正落地**（此前方案/名单导入导出只写内部 `__clip__` 存储键，没接剪贴板/文件=未实现）。`PluginRuntime` 新增桥接 `starhope.saveText(filename,text)`(系统保存对话框写文件,回调 `onSaved`)、`starhope.pickTextFile()`(文件选择框读文件,回调 `onImportFile(content|null)`)、`starhope.clipboard.setText`(真系统剪贴板)；`_callback(fn,args)` helper 回调插件全局函数。插件侧导出走 saveText、导入走 pickTextFile+onImportFile 按 `_importKind` 分发(applyImportPrizes/Snapshot/Names)、模板复制走 clipboard，**彻底移除 `__clip__` 占位**。版本 bump 18→19(sha256/size 同步)，运行副本已同步(保留 storage.json)。analyze 0 / build 通过 / 启动 124 / Node debug 全过(含文件导入导出)。
> **更新（2026-07-23 · 导航修复）**：错题本「练这些错题」/ 考试「重练错题」push 练习页后，答题完回到练习设置页**无返回按钮**——因这两处用 `FrostedBackground(child: PracticePage())` 只包背景没包带返回的标题栏。修复：把 `learning_tools_page` 私有 `_ToolHost` 提升为公开 `ToolHostPage`(common/tool_host.dart：FrostedBackground+WindowTitleBar 带返回+子页)，三处 push(学习工具/错题本/考试)统一用它。排查：学习工具栏 5 页经 _ToolHost 均有返回、ReaderViewerPage 自带返回，仅这两处遗漏已修。analyze 0 / build 通过 / 启动 124。
> **更新（2026-07-23 · 插件 v20）**：抽奖&点名插件**中签统计/抽奖历史/点名历史支持单个+批量操作**。历史条目带 id（recordWinners/rollCall 加 id，ensureHistIds 给旧数据补）；抽奖/点名历史：常态每条单删 + 顶部清空全部/批量操作，选择模式支持全选/删除选中/导出选中(剪贴板)/完成；中签统计：每条奖品清零 + 清零全部。onAction 用 ':' split 分发 histMode/histToggle/histAll/histDel/histDelSel/histExpSel/resetCount。版本 19→20(sha256/size 同步)，运行副本已同步(保留 storage.json)，仅 JS 改动无需 rebuild，Node debug 全过。
...
**必须维持的绕过（详见 `C:\Users\14254\.claude\projects\d--Personnel-StarHope\memory\starhope-build-workarounds.md`）：**
...
3. **（规划）与 Deep-tutor 后端 API 集成**：StarHope 桌面端调用 Deep-tutor backend API（云同步/在线题库等）。两端现为各自独立仓库（StarHope 已独立出 Deep-tutor monorepo），API 契约需跨仓库协同。
...
lib/
  ...
  views/
    ...
    auth/
      ...
      register_page.dart
    home/
      home_shell.dart           # ★ 底部圆边毛玻璃导航（StarBottomNav 5 项）+ 内容 FadeThroughSwitcher；HomeTab: 学习工具/插件/摘要/AI/设置
...
## 七、立即接续建议
1. 先读 `C:\Users\14254\.claude\projects\d--Personnel-StarHope\memory\` 下两个 memory 文件（架构 + 构建绕过）。
